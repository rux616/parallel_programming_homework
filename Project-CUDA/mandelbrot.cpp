...
// defining an RGB struct to color the pixel
struct Type_rgb
{
    float r;
    float g;
...
// pixel variable contain the value of the color pixel in the picture. pattern is a predefine set of
// color for a particular value.
struct Type_rgb pixels[841 * 1440], pattern[999];

// function mandelbrotset find where the number is in mandelbrotset or not and also assign a color
// to that coordinate with that iteration pattern.
__global__ void mandelbrotset()
{
    ...
    // max_iteration :- maximum number of iteration (which is one of bailout condition)
    ...
    for (y0 = -1; y0 < 1.1; y0 = y0 + 0.0025)
    {
        for (x0 = -2.5; x0 < 1.1; x0 = x0 + 0.0025)
        {
            ...
            // (x*x) + (y*y) < (2*2) is the 2nd bailout condition ie the mandelbrot set is always
            ...
            {
                // setting color pixel to the reset of the coordinate by the pattern of no of
                // iteration before bailout.
